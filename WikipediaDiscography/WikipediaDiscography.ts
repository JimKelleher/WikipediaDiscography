...
class wikipedia_discography_startup {

    // Runtime arguments:
    artist_array: string[];
    offline_discographies_json: string;
    online: boolean = true; // init
    ...
    wikipedia_discography_argument(url_search: string): void {
        ...
        let url_search_edited: string = url_search;
        ...
        if (this.online == true) {
            ...
            let search_array: string[] = url_search_edited.split("&");

            // Offload Artist names from the array in URL format to an array in Wikipedia-ready format:
            for (let i: number = 0; i < search_array.length; ++i) {

                // Isolate the artist's name and load the Wikipedia-ready format:
                let index: number = search_array[i].indexOf("artist=");

                if (index > -1) {
                    this.artist_array.push(
                        decodeURIComponent(search_array[i].substring(index + 7))
                    );
                }
    ...
    get_artists_discographies_core(discography_online: boolean, offline_discographies: string, artist_array: string[], output_format: string) {

        // NOTE: This string may eventually contain either JSON or HTML:
        let return_string: string = ""; // init

        // Initialize the Wikipedia Discography services object
        let wikipedia_discography_services: wikipedia_discography = new wikipedia_discography(
            discography_online,
        ...
        // of a JSON string:
        let artists_discographies_JSON_string: string =
...
const DISCOGRAPHY_INDICATOR1: string = "==Discography==";
const SUBSECTION_START_INDICATOR: string = "\\n'''";
const DISCOGRAPHY_SPLIT_INDICATOR1: string = "\\n*";
const DISCOGRAPHY_SPLIT_INDICATOR2: string = "n*";

// This is a "compromise" search string which also handles
// "'''" (3 single quotes):
const SUBSECTION_END_INDICATOR: string = "'";

const SECTION_STYLE_A_START_IND_WITH_EQUAL: string = "\\n\\n==="; // indicator
const SECTION_STYLE_A_END_IND_WITH_EQUAL: string = "===\\n*"; // indicator

const SECTION_STYLE_B_START_IND_WITH_SEMI: string = "\\n\\n;"; // indicator
const SECTION_STYLE_B_END_IND: string = "\\n*"; // indicator

const SECTION_TITLE_START_INDICATOR: string = "\\n==="; // \\n = \n (escaped)

const WIKITABLE_INDICATOR: string = '{| class=\\"wikitable\\"';

const STRING_LEADER: string = "the ";
const STRING_TRAILER: string = " band";

const DISCOGRAPHY_INDICATOR2: string = "== Discography ==";
const DISCOGRAPHY_INDICATOR3: string = "==Selected discography ==";
const DISCOGRAPHY_INDICATOR4: string = "==Solo discography==";
const DISCOGRAPHY_INDICATOR5: string = "==Discography and chart history==";
const DISCOGRAPHY_INDICATOR6: string = "==Partial discography==";
const DISCOGRAPHY_INDICATOR7: string = "==Selected discography==";
const DISCOGRAPHY_INDICATOR8: string = "==U.S. discography==";

// NOTE: These are special cases (see NOTEs, below):
const DISCOGRAPHY_INDICATOR20: string = "==Studio albums==";
const DISCOGRAPHY_INDICATOR21: string = "==Albums==";
const DISCOGRAPHY_INDICATOR22: string = "== Studio albums ==";
...
const WIKIPEDIA_CLEANUP_FLAGGED: string = "formatting does not conform to Wikipedia standard";

// Space character, lower case "a-z", upper case "A-Z" and all digits:
const WIKIPEDIA_ALBUM_VALID_REGULAR: string = "\\sA-Za-z0-9";

// Regularize the detail line row ends:
const DISCOGRAPHY_SPLIT_INDICATOR3: string = "-\\n"; // \\n = \n (escaped)
const WIKITABLE_ROW_END_INDICATOR: string = "-\\n|";

const MONTH: string[] = ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"];
const CHART: string[] = ["UK", "US", "Nor", "Aus"];
const RANK_UPPER_LIMIT: number = 250;

const DAY_UPPER_LIMIT: number = 31;

// NOTE: Since the other, unbroken, section uses this section indicator, I will do likewise:
const SECTION_INDICATOR: string = "==";

// Classes:

// This is returned by level_6_get_discog_array_core_inclusion_test():
class DiscographyDetailCore {
    yearArray: number[];
    titleArray: string[];
}

// This is returned by get_discography_arrays_from_JSON_string():
class DiscographyDetailJson {
    studioAlbumArray: string[];
    concertAlbumArray: string[];
}
...
class wikipedia_discography {

    // Typescript type assignments:
    ...
    artist: string = "";
    style: number = -1;
    use_external_discog: boolean = false;
    ...
    discography_return_JSON_string: string = "";

    // These are the output processing "control break" fields:
    previous_artist: string = "";
    ...
    style_0_count: number = 0;
    style_1_count: number = 0;
    style_2_count: number = 0;
    style_3_count: number = 0;
    style_4_count: number = 0;
    style_5_count: number = 0;
    style_6_count: number = 0;
    style_7_count: number = 0;
    style_8_count: number = 0;
    style_61_count: number = 0;
    style_62_count: number = 0;
    style_63_count: number = 0;
    style_64_count: number = 0;
    style_65_count: number = 0;
    style_67_count: number = 0;
    style_68_count: number = 0;
    ...
    debug_trace_artist: boolean = false;
    debug_trace_discography: boolean = false;
    debug_trace_section: boolean = false;
    debug_trace_album: boolean = false;
    ...
    simulate_album_errors: boolean = false;

    // 3 of 5: Optional processing to simulate an unexpected error:
    simulate_unexpected_error: boolean = false;
    ...
    debug_get_distinct_sections: boolean = false;

    debug_distinct_sections_all: string[];
    debug_distinct_sections_selected: string[];
    ...
    show_eponymous_titles: boolean = false;

    constructor(online: boolean, offline_discographies: string) {
    ...
    get_discographies_JSON_string(artist_array: string[], format: string): string {
    ...
    get_discography_arrays_from_JSON_string(artist_string: string, JSON_response_string: string, studio_selected: boolean, concert_selected: boolean): DiscographyDetailJson {

        let studio_album_array: string[] = [];
        let concert_album_array: string[] = [];

        let is_concert_section: boolean;
        let is_concert_album: boolean;

        // Convert the "flat" JSON string to a "live" JSON object so we can examine the contents of the response:
        let JSON_response_object: ArtistDiscography;
        ...
        let albums: string[];
        let distinct_albums: string[];

        // Level 1 of 3, the Artist loop using the "i" index:
        for (let i: number = 0; i < JSON_response_object.artistDiscog.length; i++) {
            ...
            if (JSON_response_object.artistDiscog[i].id == artist_string) {
                ...
                for (let j: number = 0; j < JSON_response_object.artistDiscog[i].section.length; j++) {
                    ...
                    for (let k: number = 0; k < JSON_response_object.artistDiscog[i].section[j].album.length; k++) {

                        is_concert_album = false;
                        if (is_concert_section == false) {

                            if (
                                this.is_concert(JSON_response_object.artistDiscog[i].section[j].album[k].albumId) == true &&
        ...
        let discography_detail_json_object: DiscographyDetailJson = { studioAlbumArray: studio_album_array, concertAlbumArray: concert_album_array };
    ...
    get_HTML_report_from_JSON_string(JSON_string: string): string {

        let artist_discography_object: ArtistDiscography;
        ...
        let report_HTML_string: string = ""; // init

        this.album_count = 0;

        // Level 1 of 3, the Artist loop using the "i" index:
        for (let i: number = 0; i < artist_discography_object.artistDiscog.length; i++) {
            ...
            // Level 2 of 3, the Section loop using the "j" index:
            for (let j: number = 0; j < artist_discography_object.artistDiscog[i].section.length; j++) {
                ...
                // Level 3 of 3, the Album loop using the "k" index:
                for (let k: number = 0; k < artist_discography_object.artistDiscog[i].section[j].album.length; k++) {
        ...
        if (this.online == true) {
            ...
            let counts_HTML_string: string = "Albums: " + this.album_count.toString() + "<br><br>"; // init
    ...
    convert_sections_style_B_to_A(discog_wikipedia_string: string): string {

        // Since we will be modifying the argument, let's make a copy and work with that:
        let discography_JSON_string_edited: string = discog_wikipedia_string;
        ...
        // Declare/init:
        let section_start_index: number = 0;
        let section_end_index: number = 0;
    ...
    convert_subsections_to_style_B_sections(discog_wikipedia_string: string): string {

        if (this.debug_trace_discography == true) { get_function_description("convert_subsections_to_style_B_sections"); }
        ...
        let subsection_array: string[] = this.get_discog_subsection_array(discography_JSON_string_edited);

        // If there are...
        if (subsection_array.length > 0) {

            //...convert subsections to sections...
            for (let i: number = 0; i < subsection_array.length; i++) {

                //...by way of the indicators that delimit them:
                discography_JSON_string_edited =
                    discography_JSON_string_edited.split(
                        SUBSECTION_START_INDICATOR +
                        subsection_array[i] +
                        SUBSECTION_END_INDICATOR
                    ).join(
                        SECTION_STYLE_B_START_IND_WITH_SEMI +
                        subsection_array[i] +
        ...
        if (this.debug_trace_discography == true) { this.write_debug("convert_subsections_to_style_B_sections AFTER: " + discography_JSON_string_edited); }
    ...
    crush(look_in: string): string {

        // Since we will be modifying the argument, let's make a copy and work with that:
        let look_in_edited: string = look_in;
    ...
    crush_artist(artist: string): string {

        // Since we will be modifying the argument, let's make a copy and work with that:
        let artist_edited: string = artist;
    ...
    debug_trace_album_arrays(section: string, year_array: number[], title_array: string[]): void {
    ...
    debug_trace_album_arrays_duplicates(section: string, year_array: number[], title_array: string[]): void {
        ...
        let title_array_work: string[] = title_array;
    ...
    debug_trace_album_arrays_range_size(section: string, year_array: number[], title_array: string[]): void {
        ...
        let year_now: number = date_now.getFullYear();
        ...
        let future_year: number = year_now + 5;

        for (let i: number = 0; i < year_array.length; i++) {
    ...
    debug_trace_album_arrays_special_chars(section: string, year_array: number[], title_array: string[]): void {
        ...
        let wikipedia_album_valid_special: string = "-!?&':,.\/";
        ...
        let wikipedia_album_valid: string = "[" + WIKIPEDIA_ALBUM_VALID_REGULAR + wikipedia_album_valid_special + "]";
        ...
        for (let i: number = 0; i < title_array.length; i++) {
            ...
            let title_char_array: string[] = title_array[i].match(new RegExp(wikipedia_album_valid, "g"));
            let title_string: string = title_char_array.join("");
    ...
    debug_trace_show_unicode_chars(look_in: string): void {
        ...
        let hex_array: string[] = look_in.split("\\u");

        // Strip down to just the unicode characters:
        for (let i: number = 0; i < hex_array.length; i++) { hex_array[i] = hex_array[i].substr(0, 4); }
        ...
        for (let i: number = 0; i < hex_array.length; i++) {
    ...
    distinct_sections(sections: string[], section: string): void {
    ...
    fix_broken_details(discog_wikipedia_array: string[]): string[] {

        // Since we will be modifying the argument, let's make a copy and work with that:
        let discography_array_edited: string[] = discog_wikipedia_array;
        ...
        let title_start_indicator_index: number;
        let title_start_indicator_index2: number;
        let title_end_indicator_index: number;

        // For each array element:
        for (let i: number = 0; i < discography_array_edited.length; i++) {
    ...
    fix_detail_indicators(discog_wikipedia_string: string): string {
    ...
    fix_detail_years(discog_wikipedia_string: string): string {

        // Since we will be modifying the argument, let's make a copy and work with that:
        let discography_JSON_string_edited = discog_wikipedia_string;

        let start_index: number = 0; // init
        ...
        while (start_index > -1) {
            ...
            if (start_index > -1) {
                ...
                let end_index: number = discography_JSON_string_edited.indexOf(")", start_index);

                if (end_index > -1) {

                    let date_to_fix: string = get_substring(discography_JSON_string_edited, start_index, end_index); // eg: (1995, Seed)
                    let fixed_date: string = get_substring(date_to_fix, 0, 4) + ")";                                 // eg: (1995)
        ...
        for (let i: number = 0; i < MONTH.length; i++) { discography_JSON_string_edited = discography_JSON_string_edited.split("(" + MONTH[i] + " ").join("("); }

        for (let i: number = 0; i < CHART.length; i++) {

            // NOTE: Reverse order prevents the accidental result of partial matching, eg: the "US #1" search hitting on "US #159":
            for (let j: number = RANK_UPPER_LIMIT; j > 0; j--) {
                discography_JSON_string_edited =
        ...
        for (let i: number = 0; i < MONTH.length; i++) {
            ...
            for (let j: number = DAY_UPPER_LIMIT; j > 0; j--) { discography_JSON_string_edited = discography_JSON_string_edited.split(j.toString() + " " + MONTH[i] + " ").join(""); }
        }
    ...
    fix_or_remove_detail_anomalies_via_hardcodes(discog_wikipedia_string: string): string {
    ...
    fix_sections(discog_wikipedia_string: string): string {

        // Built-in debugging:
        if (this.debug_trace_discography == true) { get_function_description("fix_sections"); }
        ...
        // Built-in debugging:
        if (this.debug_trace_discography == true) { this.write_debug("fix_sections AFTER<br>" + discography_JSON_string_edited); }
    ...
    get_artist_name_core(artist_name: string): string {
        ...
        if (artist_name == "the the" || artist_name == "the band") {
            ...
            let artist_edited_word_array: string[] = artist_name.split(" ");

            // Remove leading or trailing spaces from all the individual words in the
            // artists name:
            for (let i: number = 0; i < artist_edited_word_array.length; i++) {
                artist_edited_word_array[i] = artist_edited_word_array[i].trim();
            }

            // Look for the presence of leaders and trailers:
            let leader_found: boolean = false;
            let trailer_found: boolean = false;

            let start_index: number = 0;
            let end_index: number = artist_edited_word_array.length - 1;
            ...
            let array_leader: string = STRING_LEADER.trim();
            let array_trailer: string = STRING_TRAILER.trim();
    ...
    get_artist_name_full(artist_name_core: string, album_or_video_title: string): string {
        ...
        let artist_name_full: string = ""; // init
        ...
        if (artist_name_core == "the the" || artist_name_core == "the band") {
            ...
            let artist_name_core_start_index: number = album_or_video_title.indexOf(artist_name_core);
            ...
            if (artist_name_core_start_index > -1) {
                ...
                // Find the bounds:
                let start_index: number = artist_name_core_start_index - STRING_LEADER.length;
                if (start_index < 0) { start_index = 0; }

                let end_index: number = artist_name_core_start_index + (artist_name_core.length - 1) + STRING_TRAILER.length;
                if (end_index > album_or_video_title.length - 1) { end_index = album_or_video_title.length - 1; }

                // Excise the candidate string:
                let artist_name_work: string = album_or_video_title.substring(start_index, end_index + 1);
                ...
                let leader_look: string = artist_name_work.substr(0, STRING_LEADER.length);
                let trailer_look: string = artist_name_work.substr(artist_name_work.length - STRING_TRAILER.length);
                ...
                if (leader_look.toLowerCase() == STRING_LEADER.toLowerCase()) { leader_found = true; }
                if (trailer_look.toLowerCase() == STRING_TRAILER.toLowerCase()) { trailer_found = true; }
                ...
                if (leader_found == true) { start_index = start_index - STRING_LEADER.length; }
                if (trailer_found == true) { end_index = end_index + STRING_TRAILER.length; }
    ...
    get_discog_subsection_array(discog_wikipedia_string: string): string[] {

        if (this.debug_trace_section == true) { get_function_description("get_discog_subsection_array"); }
        ...
        let subsection_array: string[] = [];

        let driver_index: number = 0; // init

        let subsection_start_index: number;
        let subsection_end_index: number;
                ...
                if (subsection_end_index > -1) {
                    ...
                    subsection_array.push(
                        ...
                        discog_wikipedia_string.substring(
                            subsection_start_index + SUBSECTION_START_INDICATOR.length,
    ...
    get_discog_wikipedia_string(artist: string, discog_wikipedia_string: string): string {

        if (this.debug_trace_discography == true) { get_function_description("get_discog_wikipedia_string"); }

        let discog_wikipedia_string_return: string = ""; // init
        let discog_literal_to_search_for: string = "";   // init

        let page_halves_array: string[];
        ...
        if (this.artist == "Jeff Beck" &&
            ...
            discog_wikipedia_string = insert_at(discog_wikipedia_string,
                ...
                discog_wikipedia_string.indexOf(WIKITABLE_INDICATOR)
            )
        ...
        if (this.test_against_external_discographies(artist) == true) {
            ...
            let the_index: number = discog_wikipedia_string.indexOf("\\n\\n==");
    ...
    get_discographies_JSON_string_attempt(artist_array: string[], format: string): string {
        ...
        let discographies_JSON_string: string;
    ...
    is_concert(wikipedia_album_title: string): boolean {

        let return_value: boolean = false; // init

        let concert_indicator: string[] = ["concert", "live"];

        for (let i: number = 0; i < concert_indicator.length; i++) {
    ...
    is_title_eponymous(artist: string, title: string): boolean {
        ...
        let artist_core: string = this.get_artist_name_core(artist);
        ...
        let artist_name_full: string = this.get_artist_name_full(artist_core, title);

        let title_core: string = this.get_artist_name_core(title);
    ...
    //------------------------------------------------------------------------------------------------------------------------
    level_1_artists(artist_array: string[]): void {
        ...
        // Init:
        let wikipedia_entry: string = "";
        let discography_response: string = "";

        let artist_variation: string;
        let discography_JSON_string: string;
        ...
        let discography_rollback_JSON_string: string;
        let discography_test_JSON_string: string;

        let discography_found_count: number = 0; // init

        //=======================================================================================================================
        // For each artist in the JimRadio Artist database...
        for (let i: number = 0; i < artist_array.length; i++) {
            ...
            if (discography_JSON_string != "") {
                ...
                let discography_JSON_string2: string = get_discog_JSON_string_hardcoded_studio_albums(artist_array[i]);
        ...
        if (this.debug_get_distinct_sections == true) {
            ...
            for (let i: number = 0; i < this.debug_distinct_sections_all.length; i++) {
            ...
            for (let i: number = 0; i < this.debug_distinct_sections_selected.length; i++) {
    ...
    level_2_artist_variations(artist: string): string {
        ...
        for (let i: number = 0; i < artist_suffix.length; i++) {
    ...
    level_3_artist(artist: string, artist_variation: string): boolean {
        ...
        let artist_formatted: string;
        let http_get_result: string;
        let page_halves_array: string[];
        ...
        let wikipedia_discog_url_portion: string;
        ...
        let wikipedia: string = "http://en.wikipedia.org/wiki/" + wikipedia_discog_url_portion;
        this.wikipedia_page_url = wikipedia;

        // Assemble:
        let uri: string = WIKIPEDIA_PREFIX + wikipedia_discog_url_portion + WIKIPEDIA_SUFFIX;

        let correct_wikipedia_entry_found: boolean = false; // init
    ...
    level_4_response(wikipedia_response_JSON_string: string): boolean {
        ...
        let wikipedia_response_object: WikipediaResponse;
    ...
    level_5_discography(discog_wikipedia_string: string): void {
        ...
        // Since we will be modifying the argument, let's make a copy and work with that:
        let discog_wikipedia_string_edited: string = discog_wikipedia_string;
    ...
    level_6_discography_array(discog_section_wikipedia_array: string[]): void {
        ...
        let is_wikitable: boolean;
        let discog_wikipedia_array: string[];

        // Init:
        let section: string = "";
        this.albums_or_singles = "";

        for (let i: number = 0; i < discog_section_wikipedia_array.length; i++) {
            ...
            try {
                ...
                if (this.debug_trace_section == true) { this.write_debug("<br>level_6_sections.sectionId BEFORE: " + section); }

                section = this.regularize_section_title(section);

                if (this.debug_trace_section == true) { this.write_debug("level_6_sections.sectionId AFTER: " + section); }
                        ...
                        if (section != "") {
                            ...
                            if (discog_section_wikipedia_array[i].length > 2) {
                                ...
                                if (this.debug_trace_section == true) {
                                    ...
                                    for (let j: number = 0; j < discog_wikipedia_array.length; j++) {
                                ...
                                let discography_detail_core_object: DiscographyDetailCore = this.level_6_get_discog_array_core_inclusion_test(this.artist, is_wikitable, section, discog_wikipedia_array);
                                   ...
                                   // If we have any detail results:
                                    if (discography_detail_core_object.yearArray.length > 0) {
                                        ...
                                        this.level_7_albums(
                                            ...
                                            discography_detail_core_object.yearArray,
                                            discography_detail_core_object.titleArray
    ...
    level_6_get_discog_array_core_inclusion_test(artist: string, is_wikitable: boolean, section: string, discog_wikipedia_array: string[]): DiscographyDetailCore {
        ...
        let album_release_year: number;
        let album_title: string;

        let year_char_look_ahead: string;
      //let year_char_look_behind: string;
        let year_char_look_behind2: string;

        // These arrays will hold our final results:
        let discography_year_array: number[] = [];
        let discography_title_array: string[] = [];

        let year_start_index: number;

        // Most, but not all, detail rows will contain album data. The following
        // winnowing will focus only on those that do:
        for (let i: number = 0; i < discog_wikipedia_array.length; i++) {
            ...
            if (discog_wikipedia_array[i].indexOf("\\n!") == -1) {
                ...
                // Get title info:
                let title_start_indicator_index: number = discog_wikipedia_array[i].indexOf("[[");
                let title_end_indicator_index: number = discog_wikipedia_array[i].indexOf("]]");
        ...
        for (let i: number = 0; i < discography_title_array.length; i++) {
        ...
        if (discography_title_array.length > 0) {
            ...
            let discography_title_array_output: string[] = [];
            let discography_year_array_output: number[] = [];
        ...
        //----------------------------------------------------------------------------------------------------------------------------
        // Package up the two resultant arrays:
        let discography_detail_core_object: DiscographyDetailCore = { yearArray: discography_year_array, titleArray: discography_title_array };
    ...
    level_7_albums(section: string, year_array: number[], title_array: string[]): void {
    ...
    regularize_album_title(artist: string, album_title: string): string {

        // Since we will be modifying the argument, let's make a copy and work with that:
        let album_title_edited: string = album_title;
    ...
    regularize_detail_row_ends(discog_wikipedia_string: string): string {
    ...
    regularize_discog_string_style_5(discog_wikipedia_string: string): string {
    ...
    regularize_section_title(section_title: string): string {

        // Since we will be modifying the argument, let's make a copy and work with that:
        let section_title_edited: string = section_title;
    ...
    remove_extraneous(discog_wikipedia_string: string): string {
        ...
        for (let i: number = 1; i <= 20; i++) { discography_JSON_string_edited = remove_all_procedural(discography_JSON_string_edited, 'rowspan=\\"' + i.toString() + '\\"'); }
    ...
    remove_non_album_chart(wikipedia_discography: string): string {
    ...
    remove_non_album_companies_or_orgs(wikipedia_discography: string): string {
    ...
    remove_non_album_international(wikipedia_discography: string): string {
    ...
    remove_non_album_media(wikipedia_discography: string): string {
    ...
    remove_non_album_misc(wikipedia_discography: string): string {
    ...
    remove_non_album_values(wikipedia_discography: string, reference_array: string[]): string {

        if (this.debug_trace_discography == true) { get_function_description("remove_non_album_values"); }

        // Since we will be modifying the argument, let's make a copy and work with that:
        let wikipedia_discography_edited: string = wikipedia_discography;

        let start_index: number;
        let end_index: number;

        for (let i: number = 0; i < reference_array.length; i++) {
    ...
    remove_replace_unicode(discog_wikipedia_string: string): string {
        ...
        // Parse to an array consisting of the remaining unicode characters, if there are any:
        let unicode_hex_array: string[] = discography_JSON_string_edited.split("\\u");

        // Strip down to just the unicode characters:
        for (let i: number = 0; i < unicode_hex_array.length; i++) { unicode_hex_array[i] = unicode_hex_array[i].substr(0, 4); }
        ...
        let unicode_ASCII_services: unicode_ASCII = new unicode_ASCII();

        // Show them to the developer...
        for (let i: number = 0; i < unicode_hex_array.length; i++) {

            if (unicode_hex_array[i].indexOf("\\n") == -1) {

                let unicode_hex: string = encodeURI("%u" + unicode_hex_array[i]);
                let unicode_hex_string: string = "\\u" + unicode_hex_array[i];

                let ASCII: string = unicode_ASCII_services.get_ASCII(unicode_hex_array[i]);
    ...
    section_or_album_title_common_cleanup(title: string): string {

        // Since we will be modifying the argument, let's make a copy and work with that:
        let title_edited: string = title;
    ...
    test_against_external_discographies(artist: string): boolean {

        let external_discography_artist: string[] = [
        ...
        let use_external_discography_artist: boolean = false; // init

        for (let i: number = 0; i < external_discography_artist.length; i++) {
    ...
    test_against_invalid_concert_album_name(album: string): boolean {
        ...
        let should_accept_album: boolean = true; // init
    ...
    test_against_non_album_sections(section: string): boolean {
        ...
        let non_album_section: string[] = [
            ...
            "video game",
            "videography",
            "videos",               // here we want the plural to eliminate "videos" but keep "video albums"
            "walk of fame",
            "web",
        ...
        for (let i: number = 0; i < non_album_section.length; i++) {
